 ...
 *Created by @shahin
 */
public class CarouselLayoutManagerCustom extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {

    private static final String TAG = "CarouselLayoutManagerCu";
    ...
    public CarouselLayoutManagerCustom(final int orientation) {
     ...
     */
    @SuppressWarnings("unused")
    public CarouselLayoutManagerCustom(final int orientation, final boolean circleLayout) {
...
//        RecyclerView.SmoothScroller linearSmoothScroller = new LinearSmoothScroller(recyclerView.getContext()) {
...
//        };
    ...
    private void fillDataVertical(final RecyclerView.Recycler recycler, final int width, final int height) {
        ...
        for (int i = 0, count = mLayoutHelper.mLayoutOrder.length; i < count; ++i) {
            ...
            Log.d(TAG, "fillDataVertical: top " + top + " bottom " + bottom + " i " + i);
    ...
    private void fillDataHorizontal(final RecyclerView.Recycler recycler, final int width, final int height) {
        ...
        for (int i = 0, count = mLayoutHelper.mLayoutOrder.length; i < count; ++i) {
            ...
            final int end = start + mDecoratedChildWidth;
            Log.d(TAG, "fillDataHorizontal: top bottom " + top + " " + bottom);
            fillChildItem(start, top, end, bottom, layoutOrder, recycler, i);
        }
    }
    ...
    private void fillChildItem(final int start, final int top, final int end, final int bottom, @NonNull final LayoutOrder layoutOrder, @NonNull final RecyclerView.Recycler recycler, final int i) {
        ...
        ItemTransformationCustom transformation = null;
        if (null != mViewPostLayout) {
            Log.d(TAG, "fillChildItem: 1");
        ...
        if (null == transformation) {
            Log.d(TAG, "fillChildItem: 2");
            view.layout(start, top, end, bottom);
        } else {
            Log.d(TAG, "fillChildItem: 3");
    ...
    private int getMaxScrollOffset() {
        return getScrollItemSize() * (mItemsCount / 2 - 1);
    }
    ...
    private void generateLayoutOrder(final float currentScrollPosition, @NonNull final RecyclerView.State state) {
        ...
        if (mCircleLayout && 1 < mItemsCount) {
            ...
            for (int i = 1; i <= countLayoutHalf; ++i) {
                ...
                Log.d(TAG, "generateLayoutOrder: centerItem " + centerItem + " absCurrentScrollPosition " + absCurrentScrollPosition + " i " + i);

                mLayoutHelper.setLayoutOrder(countLayoutHalf - i, position, centerItem - absCurrentScrollPosition - i - 0.3f);

//                if (i>0) {
//                    Log.d(TAG, "generateLayoutOrder: i top  "+i);
//                    mLayoutHelper.setLayoutOrder(countLayoutHalf - i, position, centerItem - absCurrentScrollPosition - i - 0.3f);
//                } else {
//                    Log.d(TAG, "generateLayoutOrder: i top else "+i);
//
//                    mLayoutHelper.setLayoutOrder(countLayoutHalf - i, position, centerItem - absCurrentScrollPosition - i+0.1f );
//
//                }
            }
            // after center item
            for (int i = layoutCount - 1; i >= countLayoutHalf + 1; --i) {
                ...
                Log.d(TAG, "generateLayoutOrder: centerItem  2   " + centerItem + " absCurrentScrollPosition " + absCurrentScrollPosition + " i " + i + "  layoutCount " + layoutCount);
                mLayoutHelper.setLayoutOrder(i - 1, position, centerItem - absCurrentScrollPosition + layoutCount - i + 0.3f);

//                if (i-1==layoutCount - 2) {
//                    mLayoutHelper.setLayoutOrder(i-1, position, centerItem - absCurrentScrollPosition + layoutCount - i+0.8f );
//
//                    Log.d(TAG, "generateLayoutOrder: i bottom  "+i);
//                } else {
//                    Log.d(TAG, "generateLayoutOrder: i bottom  else "+i);
//                    mLayoutHelper.setLayoutOrder(i - 1, position, centerItem - absCurrentScrollPosition + layoutCount - i +0.2f);
//
//                }
            ...
            Log.d(TAG, "generateLayoutOrder: else ");
    ...
    protected int getCardOffsetByPositionDiff(final float itemPositionDiff) {
        ...
        final int dimenDiff;
        if (VERTICAL == mOrientation) {
            dimenDiff = (int) ((getHeightNoPadding() - mDecoratedChildHeight) / 1.7f);
        } else {
            dimenDiff = (int) ((getWidthNoPadding() - mDecoratedChildWidth) / 1.6f);
        }
        //noinspection NumericCastThatLosesPrecision
    ...
    protected double convertItemPositionDiffToSmoothPositionDiff(final float itemPositionDiff) {
        ...
        if (absIemPositionDiff > StrictMath.pow(1.0f / mLayoutHelper.mMaxVisibleItems, 1.0f / 0.5)) {
            // this item is far from center line, so we should make it move like square root function //STandard 1.12 else
            return StrictMath.pow(absIemPositionDiff / mLayoutHelper.mMaxVisibleItems, 1 / 1.0f);
        } else {
    ...
    public abstract static class PostLayoutListener {
         ...
         */
        public ItemTransformationCustom transformChild(
